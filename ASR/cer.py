...
def compute_cer(ref, pred, progress_bar=True):
    if progress_bar:
        pbar = tqdm(total=len(ref), desc="Computing CER")
    else:
        pbar = None
...
if args.metric == 'cer':
    ...
    print(f"spaced CER for {args.dataset} : {space_cer} \n CER without space : {no_space_cer}")

else:
    true = read_wer_texts(args.true_path)
    pred = read_wer_texts(args.pred_path)
    wer = compute_cer(true, pred)
    print(f"WER for {args.dataset} : {wer}")
